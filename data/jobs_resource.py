...
class JobsResource(Resource):
    def get(self, job_id):
        ...
        session = db_session.create_session()
        jobs = session.query(User).get(job_id)
...
class JobsListResource(Resource):
    ...
    def post(self):
        ...
        jobs = Jobs(
            job=args.json['job'],
            team_leader=args.json['team_leader'],
            work_size=args.json['work_size'],
            is_finished=args.json['is_finished'],
            collaborators=args.json['collaborators']
        )
        session.add(jobs)
        session.commit()
        return jsonify({'success': 'OK'})
